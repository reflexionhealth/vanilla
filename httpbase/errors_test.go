...
func TestError(t *testing.T) {
	r := router.New()
	r.GET("/", func(c *router.Context) { Error(&c.Response, 418, "Teapots cannot make coffee") })

	w := routertest.PerformRequest(r, "GET", "/")
...
func TestUnauthorized(t *testing.T) {
	r := router.New()
	r.GET("/", func(c *router.Context) { Unauthorized(&c.Response) })
...
func TestForbidden(t *testing.T) {
	r := router.New()
	r.GET("/", func(c *router.Context) { Forbidden(&c.Response) })
...
func TestNotFound(t *testing.T) {
	r := router.New()
	r.GET("/", func(c *router.Context) { NotFound(&c.Response) })
...
func TestNoMethod(t *testing.T) {
	r := router.New()
	r.GET("/", func(c *router.Context) { NoMethod(&c.Response) })
